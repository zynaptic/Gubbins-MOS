...
/*
 * Implements GubbinsMOS debug serial console support using USART1 and
 * DMA1 channel 4.
 */
...
void gmosPalSerialConsoleInit (void)
{
    ...
    // Enable clocks for USART1 and DMA1. Note that these are not
    // enabled in the corresponding sleep mode registers, so the clocks
    // will automatically be gated on entering sleep mode.
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
    RCC->AHBENR |= RCC_AHBENR_DMA1EN;

    // Set the USART1 baud rate (8N1 format is selected by default).
    usartDiv = (((2 * GMOS_CONFIG_STM32_APB2_CLOCK) /
        GMOS_CONFIG_STM32_DEBUG_CONSOLE_BAUD_RATE) + 1) / 2;
    USART1->BRR = usartDiv;

    // Configure USART1 to use DMA based transmission.
    USART1->CR3 = USART_CR3_DMAT;

    // Set up DMA channel 4 for use with the console UART. This DMA
    // channel is the only one available for use with USART1 on all
    // device categories.
